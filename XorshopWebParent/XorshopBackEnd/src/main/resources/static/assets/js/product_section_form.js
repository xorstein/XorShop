...
function insertProductCode(productId) {
	...
	$.get(url, function(productJson) {
		...
		//productImagePath=contextPath+productImagePath1
...
function generateProductCode(productId, productName, productImagePath, nextCount) {
	...
	productCode = `
		<div class="col-sm-2 mt-4" id="${colId}" draggable="true" ondragstart="dragProduct(event)">
			<input type="hidden" name="productId" value="${productId}" class="product-id-hidden" />
			<div class="mb-3">
				<a class="fas fa-trash text-danger link-remove" href="" colNumber="${nextCount}"></a>
				&nbsp;&nbsp;&nbsp;&nbsp;
				<a class="fas fa-chevron-left text-info link-move-left" href="" title="Move this product to the left" colNumber="${nextCount}"></a>
				&nbsp;&nbsp;&nbsp;&nbsp;
				<a class="fas fa-chevron-right text-info link-move-right" href="" title="Move this product to the right" colNumber="${nextCount}"></a>				
			</div>					
