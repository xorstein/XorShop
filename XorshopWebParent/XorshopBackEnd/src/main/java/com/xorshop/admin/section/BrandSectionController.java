...
public class BrandSectionController {
	...
	public String showForm(Model model) {
		...
		model.addAttribute("pageTitle", "Section de marques - Nouvel element");
	...
	public String saveSection(Section section, HttpServletRequest request, RedirectAttributes ra) {
		...
		ra.addFlashAttribute("message", "La section de type marque a été enregistrée avec succès.");
	...
	public String editSection(@PathVariable(name = "id") Integer id, RedirectAttributes ra,
		...
		try {
			...
			model.addAttribute("listBrands", listBrands);
			model.addAttribute("section", section);
			model.addAttribute("pageTitle", "Section de marques - Modifier l'element (Numéro: " + id + ")");

			return "sections/brand_section_form";

		} catch (SectionNotFoundException ex) {
			LOGGER.info("BrandSectionController | editSection | error : " + ex.getMessage());
